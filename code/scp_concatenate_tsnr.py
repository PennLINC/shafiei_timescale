...
if dataset == 'HCPD':
    ...
    fileNames = sorted(glob.glob(inpath +
                                 ...
                                 '_den-91k_stat-mean_tsnr_%s.npy'
                                 % ds_method))
...
# generate empty array for tsnr
concat_tsnr = np.zeros((len(subjList), 400))
df_concat_qc = []
for iSubj in range(len(subjList)):
    subj_tsnr = np.load(fileNames[iSubj])
    if dataset == 'HCPYA':
        concat_tsnr[iSubj, :] = subj_tsnr[:400]
    else:
        concat_tsnr[iSubj, :] = subj_tsnr
...
concat_tsnr = np.array(concat_tsnr)
...
if dataset == 'HCPYA':
    ...
    # create dataframe
    final_df = pd.DataFrame(concat_tsnr, columns=list(mapped['output_roi']))
    final_df = pd.concat([final_df, df_main], axis=1)
else:
    # matrix permuted to 7-network ordering
    concat_tsnr_17to7 = concat_tsnr[:, mapped['mapped_indices'].values]

    # create dataframe
    final_df = pd.DataFrame(concat_tsnr_17to7,
...
final_df.to_csv(outpath + 'concat/' +
                '%s_concat_tsnr_%s_Schaefer_400-7.csv'
