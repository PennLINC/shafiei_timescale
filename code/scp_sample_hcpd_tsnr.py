...
# load timescale data
ts_data = pd.read_csv(datapath + '%s/%s_tsnr/%s/concat/'
...
outpath = datapath + '%s/%s_%s_%s_concat_tsnr/' % (dtype, dataset,
                                                   task, ds_method)
...
outpath = datapath + '%s/%s_%s_age_%s_concat_tsnr/' % (dtype, dataset,
                                                       task, ds_method)
if not os.path.exists(outpath):
    os.makedirs(outpath)

final_df_age822.to_csv(outpath + '%s_%s_age_concat_%s_%s_Schaefer_400-7'
                       % (dataset, task, metric, ds_method) +
                       '_forR.tsv',
                       index=False, sep='\t')
