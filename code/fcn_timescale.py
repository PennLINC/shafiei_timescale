...
def get_spinp(x, y, corrval, nspin, lhannot, rhannot, corrtype, surfpath):
    surf_path = surfpath
    ...
    centroids, hemiid = get_gifti_centroids(surfaces, lhannot, rhannot)
...
def get_spinidx(nspin, lhannot, rhannot, surfpath):
    surf_path = surfpath
    surfaces = [surf_path + 'L.sphere.32k_fs_LR.surf.gii',
                surf_path + 'R.sphere.32k_fs_LR.surf.gii']
    lhannot = lhannot
...
def scatterregplot(x, y, title, xlab, ylab, pointsize):
    myplot = sns.scatterplot(x=x, y=y,
...
def make_colormaps():
    ...
    cmap_PuOr = LinearSegmentedColormap.from_list('mycmap', list(
                np.array(pal.colorbrewer.diverging.PuOr_3.mpl_colors)))

    colors = ["goldenrod", "white", "#6f1282"]  # low, mid, high
    cmap_YlPu = LinearSegmentedColormap.from_list("custom_diverging",
                                                  colors, N=256)

    colors = np.vstack([np.array(cmap_seq(i)[:3]) for i in range(256)])
    megcmap = ListedColormap(colors)

    colors = np.vstack([np.array(cmap_seq_r(i)[:3]) for i in range(256)])
    megcmap_r = ListedColormap(colors)
    ...
    return (cmap_seq, cmap_seq_r, megcmap, megcmap_r, megcmap2,
            categ_cmap, cmap_OrYel, cmap_PuOr, cmap_YlPu)


def make_colormap_rsn():
    colors = sns.color_palette('hls', n_colors=7)
    categ_cmap = ListedColormap(np.vstack((np.ones((37, 3)) *
                                           np.array(colors[0]),
                                           np.ones((37, 3)) *
                                           np.array(colors[1]),
                                           np.ones((37, 3)) *
