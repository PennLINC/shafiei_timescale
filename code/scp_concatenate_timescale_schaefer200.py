...
concat_tau = np.zeros((len(subjList), 200))
df_concat_qc = []
for iSubj in range(len(subjList)):
    subj_tau = np.load(fileNames[iSubj])
    if dataset == 'HCPYA':
        concat_tau[iSubj, :] = subj_tau[:200]
    else:
...
mapped = pd.read_csv(reorder_dir + 'Schaefer_200-17_mappedto_200-7.csv')
...
final_df.to_csv(outpath + 'concat/' +
                '%s_concat_timescale_acf_%s_Schaefer_200-7.csv'
